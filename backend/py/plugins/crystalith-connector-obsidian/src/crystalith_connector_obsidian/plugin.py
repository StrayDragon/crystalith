...
class ObsidianVaultConnectorPlugin:
    api_version = "v1"
    ...
    async def get_diagnostics(
        ...
        *,
        ...
        if connection_config is None:
            return None
        _, diags = _resolve_root(connection_config, key="vault_path")
        return diags
    ...
    async def list_snapshot_entries(
        ...
        *,
        ...
        root, diags = _resolve_root(connection_config, key="vault_path")
        if root is None:
            raise ValueError(diags[0]["message"] if diags else "Invalid vault_path")
    ...
    async def read_file_bytes(
        ...
        *,
        ...
        if candidate != resolved_root and resolved_root not in candidate.parents:
            raise ValueError("relative_path escapes vault root")
...
plugin = ObsidianVaultConnectorPlugin()
