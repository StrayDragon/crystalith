...
| K16 | MINDMAP 渲染为线性树（GenericOutputRenderer）而非交互式 Graph（MindmapViewer）                                                           | ✅ 已修复 — OutputContent.tsx 添加 `typeId === 'MINDMAP'` 分支直接分发到 `<MindmapViewer>`                                                 |
| K17 | FAQ/QUIZ/GUIDE/TIMELINE/BRIEFING 全部回退到 GenericOutputRenderer 静态渲染，缺少 v1 plugin system 对应的交互式行为                       | ✅ 全部已修复 — OutputContent.tsx 依次添加每个类型的特判分支，分发到 FlashcardViewer/QuizRunner/GuideChecklist/TimelineViewer/ReportViewer |
| K18 | QUIZ 选项比较 bug：选项含字母前缀（`"B. 计算机科学"`）而答案为字母（`"B"`），`normalize` 直接比较 `"b" !== "b. 计算机科学"` 永远判定错误 | ✅ 已修复 — QuizRunner.tsx 新增 `extractOptionLetter`/`stripOptionPrefix`，比较时先尝试字母前缀匹配，CDP 验证 "回答正确"                   |
