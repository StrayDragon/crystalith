...
用户 UI 功能与 Server 路由映射。`Status`: **active** = 主路径；**orphan** = 有代码无入口；**api-only** = 仅 API 仍保留；**removed** = 已删（c73 等，勿再当活路由）。
...
| `chat-citations`                | 引用            | 嵌入 QA/messages `citations`（~~citations HTTP~~ c73 removed）         | active            | 无独立 citations 路由                             |
...
| `output-queue-jobs`             | 任务队列        | `POST .../outputs`, slides SSE（客户端本地队列）                       | active            | `/v2/tasks*` c73 已删                             |
| `output-fallback-retry`         | 降级重试        | `POST /v2/outputs`                                                     | active            | 待盘点                                            |
| `generic-output-renderer`       | 通用渲染        | `GET /v2/workspace/tools`                                              | active            | ~~`/outputs/types`~~ c73 已删                     |
| `output-note-type-paragraph`    | PARAGRAPH       | `POST /v2/outputs`                                                     | active            | ~~`/v2/refine`~~ c73 已删                         |
| `output-note-type-bullets`      | BULLETS         | `POST /v2/outputs`                                                     | active            | ~~`/v2/refine`~~ c73 已删                         |
| `output-note-type-structured`   | STRUCTURED      | `POST /v2/outputs`                                                     | active            | ~~`/v2/refine`~~ c73 已删                         |
...
| —                               | —               | `GET/POST /v2/refine*`                                                 | **removed** (c73) | 2026-07-20：无 FE；整域移除                       |
| —                               | —               | `GET /v2/qa/presets`                                                   | **removed** (c73) | 2026-07-20：路由删除；内置预设经 commands/QA 仍活 |
| —                               | —               | `GET /v2/outputs/types`                                                | **removed** (c73) | 2026-07-20：路由删除；由 workspace/tools 替代     |
| —                               | —               | `GET .../citations/context`                                            | **removed** (c73) | 2026-07-20：无 FE；整域移除                       |
| —                               | —               | `POST /v2/qa` (非流式)                                                 | api-only          | FE `useChat` 有非流式分支；默认流式               |
| —                               | —               | `GET /sources/parsers`                                                 | api-only          | 待盘点                                            |
| —                               | —               | `GET/POST /v2/tasks*`（含 cancel）                                     | **removed** (c73) | 2026-07-20：无 FE；整域移除                       |
| —                               | —               | `GET /v2/models/:modelId`, `GET /v2/models/providers`                  | api-only          | 待盘点                                            |
| —                               | —               | `GET/POST /v2/eval/*` (8 endpoints)                                    | **removed** (c73) | 2026-07-20：无 FE；整域移除                       |
| —                               | —               | `GET/POST /v2/strategies`, `.../notebooks/:nid/strategies`             | **removed** (c73) | 2026-07-20：HTTP 删除；保留内部 ragRegistry       |
