...
test('buildMarkdownExport keeps guide/timeline/briefing readable', () => {
  const guide = buildMarkdownExport(createOutput('GUIDE', {
    modules: [{ title: '模块一', objective: { text: '理解基础' }, key_points: [{ text: '要点 A' }] }],
  }));
  const timeline = buildMarkdownExport(createOutput('TIMELINE', {
    events: [{ date: '2024', event: '发布', description: '发布了新版本' }],
  }));
  const briefing = buildMarkdownExport(createOutput('BRIEFING', {
...
test('buildJsonExport outputs flashcard and quiz schemas', () => {
  const faqJson = buildJsonExport(createOutput('FAQ', {
    items: [{ question: 'Q1', answer: 'A1' }],
  }));
  const quizJson = buildJsonExport(createOutput('QUIZ', {
...
test('buildSlidesExportItems supports outline and markdown fallback', () => {
  const outlineSlides = buildSlidesExportItems(createOutput('SLIDES', {
    outline: {
      slides: [
  ...
  const markdownSlides = buildSlidesExportItems(createOutput('SLIDES', {
    markdown: '# 总览\n\n## 章节一\n- 重点一\n- 重点二\n',
  }));
