...
const MindmapEdge = memo(function MindmapEdge({ id, sourceX, sourceY, targetX, targetY, style }: EdgeProps) {
...
function buildTree(node: MindmapNode, depth: number, prefix: string, collapsed: Set<string>): LayoutNode {
  const id = prefix;
  const isCollapsed = collapsed.has(id);
  const childCount = node.children?.length || 0;
  const children = isCollapsed ? [] : (node.children || []).map((c, i) =>
    buildTree(c, depth + 1, `${prefix}-${i}`, collapsed));
...
function layoutY(node: LayoutNode, startY: number): void {
  if (node.children.length === 0) { node.y = startY; return; }
  let y = startY;
  for (const c of node.children) { layoutY(c, y); y += c.height + V_SPACING; }
...
function buildElements(data: MindmapData, collapsed: Set<string>): { nodes: Node[]; edges: Edge[] } {
...
function getAutoCollapsed(node: MindmapNode, depth: number, prefix: string, maxDepth: number): Set<string> {
...
function InnerFlow({ data, autoCollapseDepth, isDarkTheme }: InnerFlowProps) {
  ...
  const collapseAll = useCallback(() => {
    ...
    const visit = (n: MindmapNode, p: string) => {
      if (n.children?.length) { all.add(p); n.children.forEach((c, i) => visit(c, `${p}-${i}`)); }
    };
