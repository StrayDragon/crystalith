...
- Dependencies：建议接在 `cross-notebook-insight-graph`、`admin-observability-and-operations-center`、`cost-intelligence-and-model-routing-governance` 之后。

## Dependency Sketch

```mermaid
flowchart TD
  WS[Workspace Growth] --> BUDGET[Capacity Budget]
  BUDGET --> DIAG[Hotspot Diagnostics]
  DIAG --> LOAD[Load Detection]
  LOAD --> ACT{Intervention Actions}
  ACT --> ARCHIVE[Archive Suggestion]
  ACT --> REINDEX[Rebuild Index]
  ACT --> PAUSE[Pause Low-priority]
  ACT --> LIMIT[Rate Limit]
  WS --> PROG[Progressive Loading]
  c17[c17: Admin Observability] --> DIAG
  c29[c29: Cost Intelligence] --> LOAD
```
