...
- [x] 2.1 新增 `sdk/client/typescript/` 包骨架（`package.json`、构建脚本、类型声明输出、README/许可证纳入）。
- [x] 2.2 复用 `openapi-ts`（或 Fern TS generator）把 OpenAPI 生成输出定向到该包内，确保与前端应用生成策略一致。
- [x] 2.3 增加“TS SDK 漂移检查”工作流：生成后 `git diff --exit-code`，在 PR/主分支上运行。
...
- [x] 4.1 更新 `docs/content/sdk-python.md` 与 `docs/content/sdk-typescript.md`：补充“安装方式（pip/npm）+ 版本策略 + 生成来源（OpenAPI）”说明。
