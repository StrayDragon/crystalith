...
- 收口并固化 drift checks（作为仓库的常规门禁）：
  ...
  - OpenAPI/client drift（引用 `c2021`）
- 将 CI “门禁”从口头约定落到可复用的基线与可消费信号：
  - 统一 PR/main 的校验基线（后端 lint/typecheck/tests/contract/config-schema/import-layering；前端 test/lint/format/typecheck/build；以及 OpenAPI/生成 client 一致性）
  - 覆盖率报告（artifact + 可选 PR 摘要）
  - 依赖漏洞扫描（PR + 定时）
  - 分支保护与 required checks 的落地建议 + 文档/Badge（避免规则只存在于口头）
- 引入 spec ↔ code ↔ tests 的 coverage map（先做“可见”，再做“约束”）：
  - 先落一个最基础的 doc/spec/code link index，把“这个能力/文档/规格对应哪些代码路径”变成显式映射
  - 区分**人工确认链接**与**系统推断链接**，避免把猜测当事实
...
- `doc-spec-code-link-index-and-coverage-map`: drift checks 清单、doc/spec/code 显式映射、coverage map 的结构与导航入口。
...
- `quality-and-regression`: CI 基线、覆盖率/安全扫描信号与 required checks 的约束。

## Impact

- DX：会多一些检查命令，但它们能显著减少“过了两周才发现文档漂了”的返工。
- Docs/Specs：维护成本会更可控；也更容易 onboarding 新贡献者。
- CI：失败输出更可定位、信号更可消费（覆盖率与漏洞扫描不再是“有没有跑过”的黑盒）。
- Dependencies：建议与 `c2021` 的 OpenAPI/client 门禁联动推进，形成完整的“契约-产物-检查”闭环。
