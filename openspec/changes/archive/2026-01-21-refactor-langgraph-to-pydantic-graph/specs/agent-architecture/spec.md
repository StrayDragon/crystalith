...
系统 SHALL 使用 `pydantic-graph` 库进行所有 Agent 工作流编排，替代原有的 LangGraph 实现。
...
依赖声明 MUST 从 `langgraph>=0.2.0` 修改为 `pydantic-graph`（版本跟随 pydantic-ai）。
...
### Requirement: LangGraph StateGraph API
**原因**：LangGraph 依赖被完全移除，所有工作流迁移到 pydantic-graph
**迁移**：
- `StateGraph(StateType)` → `Graph(nodes=[...])`
- `graph.add_node("name", func)` → `class NodeName(BaseNode[...])`
- `graph.add_edge("a", "b")` → 节点 `run()` 返回类型
- `graph.set_entry_point("name")` → `Graph.run(StartNode(), ...)`
- `END` → `End[OutputType](value)`
- `TypedDict` 状态 → `@dataclass` 状态
