...
### Requirement: 来源锚定机制
系统 MUST确保所有 AI 回答严格基于用户提供的来源，实现 Source-Grounding，不引入外部知识。
...
### Requirement: 精确引用定位
系统 MUST提供精确到段落或页码的引用定位，支持用户追溯原文。
...
### Requirement: 无证据检测
系统 MUST检测并提示无法从来源中找到证据的情况，避免生成无根据的回答。
