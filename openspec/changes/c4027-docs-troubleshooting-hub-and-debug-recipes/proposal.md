...
我们其实已经在 specs 和 changes 里定义了不少诊断能力：startup self-test（`c1035`）、诊断工作台 + diagnostics export pack（`c2020`）、correlation id（`c2152`）……缺的是把这些能力“翻译成普通人能用的排障手册”，并放到 docs-site 的显眼位置。
...
- Dependencies：建议优先与 `c1035`、`c2020`、`c2160` 对齐术语，避免同一件事在不同地方叫不同名字。
