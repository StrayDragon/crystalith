# publishable-artifacts Specification

## Purpose

让结果从一次性 output 升级为站内可沉淀的正式产物（artifact）：支持显式 promotion、独立生命周期、lineage 可追溯、可引用标识、继续编辑与归档语义，强调站内持续工作而非外部导出。

## Non-goals

- 不把外部导出/发布写成 v1 主线（后置）
- 不自动把任意结果提升为 artifact（promotion 必须显式触发）
