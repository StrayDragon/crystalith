# http-runtime-guardrails Specification

## Purpose

定义 HTTP 入口层的最小运行时防护（upload size guard + rate limit），在非本地暴露场景默认开启，避免资源耗尽与误用，同时保持本地开发低摩擦。

## Non-goals

- 不承诺完整 DDoS/WAF 防护
- 不实现分布式限流（多副本共享 state）
- 不在该规范内建设 metrics/tracing
- 不把上传重构为流式落盘管线（仅限定最大读取字节数）
