...
定义文档站点的最小信息架构与发布方式：使用 MkDocs 以 `docs/` 为源构建静态站点，并通过 GitHub Actions 发布到 GitHub Pages，保证关键文档（入门/部署/配置/SDK/插件/贡献）可持续维护。
...
仓库根目录 MUST 存在 `mkdocs.yml`，且 MUST 存在 `docs/` 目录用于存放 Markdown 与资源文件。

### Requirement: Local build and preview
系统 MUST 支持在本地构建与预览文档站点，以便在不依赖 CI 的情况下验证文档内容与导航结构。
在仓库根目录执行 `mkdocs build` 时 MUST 生成静态站点输出目录（默认 `site/`）并包含 `index.html`；执行 `mkdocs serve` 时 MUST 在本地端口提供可访问的预览站点。
...
站点配置 MUST 引用 `docs/assets/logo.webp` 作为 logo。
