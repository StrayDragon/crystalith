...
### Requirement: SourceConnectorPlugin is supported as a first-class interface
宿主 MUST 将 `SourceConnectorPlugin` 作为与 `ParserPlugin`、`OutputTypePlugin`、`WebExtractorPlugin` 并列的一类插件接口，纳入统一的发现、兼容性门禁、启用策略与诊断失败语义。

#### Scenario: Discover source connector plugins at runtime
...
### Requirement: Source connectors reuse host workflow shell by default
宿主 MUST 为 source connector 插件提供默认的连接器工作流壳子，以避免每个插件重复开发同类 UI 与流程。

#### Scenario: Connector plugin relies on host UI
