# evidence-review-workflow Specification

## Purpose

把 citation/evidence 的核查做成正式工作流：结果级审阅状态机、citation/evidence 级核查项与 review notes 的可追溯记录，以及稳定入口与摘要字段，支撑“待审/已审/需修订”的可信闭环。

## Non-goals

- 不构建组织级多层审批系统（会签、SLA、权限流后置）
- 不把质量门信号等同于 evidence review（质量门仅可提示，不裁决）
