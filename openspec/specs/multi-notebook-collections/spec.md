# multi-notebook-collections Specification

## Purpose

在 notebook 之上提供聚合层 collection：通过绑定多个 notebooks 形成更大的工作上下文，并提供 collection-scoped 的检索与导航语义，同时保留 notebook 的身份与来源归属边界。

## Non-goals

- 不把 collection 做成新的“万能容器”吞掉 notebook 语义
- v1 不覆盖 collection 级模板/审阅/发布联动（后置）
