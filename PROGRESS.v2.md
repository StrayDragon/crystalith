...
| c24     | pipeline-integration   | 🔄 WIP (WS-A✅ / WS-C✅ 全量28 tests / WS-B拆为对齐提案)                         |
| c25     | ssrf-config            | ⬜ TODO (GAP-BOARD G15)                                                          |
| c26     | citation-context       | ⬜ TODO (GAP-BOARD G8)                                                           |
| c27     | outputs-rag            | ⬜ TODO (GAP-BOARD G7)                                                           |
| c28     | analysis-vector        | ⬜ TODO (GAP-BOARD G2)                                                           |
| c29-c35 | v1-alignment           | ⬜ TODO (GAP-BOARD G1/G3/G9/G10/G11/G12/G13/G16,编号待定)                        |
| c13     | distribution           | ⏸️ BLOCKED (blocked by c24 + 对齐提案) 🔒 需要人工授权                           |
| c14     | cleanup-delivery       | ⏸️ BLOCKED (blocked by c24 + 对齐提案) 🔒 需要人工授权                           |
...
- **Workstream B — toolApproval 事件驱动 HITL** ⏳(拆分为独立对齐提案,见下方 GAP-BOARD)：原 WS-B(research架构切换)是 v2 改进项,但全面复核发现大量 v1 行为偏移需优先对齐
- **Workstream C — 集成测试套件** 🔄：QA/sources/refine/studio/outputs ✅（25 tests）；research 5 测试待 research 对齐后补

**阻塞**: c13 (distribution/Tauri)、c14 (cleanup/delivery) 需 c24 + 对齐提案全部完成后方可开始

**完成后**: c24 → 对齐提案(c25-c30+) → c13 → c14 = v2.0.0

**策略调整(2026-07-10)**: 全面复核发现 v2 在端点(19缺失)和行为(6域偏移)上系统性偏离 v1。WS-B 不再是下一批——改为**拆分为多个对齐提案,一个一个做,不跳过,保证完整迁移**。前端迁移(~87% 未做)在对齐后端后补齐。
...
| G7  | outputs-rag        | 🟡 无RAG检索(全chunk dump)+无citations                    | `output_graph.py`                         | c27            | P0     | ⬜ 已有提案 |
...
```
P0 先行(功能不可用/数据错误):
  c29 refine 回退对齐 → c30 sources同步embedding → c28 analysis向量恢复
  → c27 outputs RAG → c24-B拆分(research resume/export/hitl)
  → c31 QA no-evidence → c35 前端迁移

P1(行为偏离):
  c26 citations-context → c32 studio(落盘+端点) → c33 sources端点

P2(体验降级):
  c25 SSRF config → c34 sessions端点 → c14 source-connectors sync
```

**注**: 编号 c29-c35 为占位,实际编号在提案创建时确定。已有提案(c25-c28)编号不变。
...
| 字段       | 值                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| :--------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 上次 Agent | pi-agent (Step 1.1 ✅ c24 WS-C research 测试 + agent eager-drain 修复)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 上次操作   | **Step 1.1 ✅**: 新增 4 个 research 集成测试文件(19 tests,全绿)。**修复 agent 关键 bug**: runResearch 原先需 SSE consumer 读流才会标记 completed→改成 eager drain + 立即持久化。全量 183 pass / 0 fail, server typecheck clean。修复过程发现并解决架构层面的 stream 消费语义问题。**下一步**: Step 1.2 c24 WS-B research parity(3 P0 缺口): (1) 端点级对拍发现 **19 个 v1 端点静默缺失**(跨 8 域:sources 3/studio 4/source-connectors 3/research 3/sessions 2/qa 1/citations 1/refine 1/models 1)。(2) 行为级对拍发现 **6 域实质偏移**: refine🔴换产品(纯文本变换器 vs citation-aware RAG 摘要器)、analysis🔴向量KNN降级为关键词TF、QA🟡架构不同(agent tool-call vs 确定性检索,5个no-evidence reason仅产出1)、research🟡resume失效(从头重跑)+export是JSON dump、outputs🟡无RAG检索、studio🟡无Slidev落盘。(3) 前端评估: c03"DONE"仅指eden脚手架,实际迁移~13%(仅notebooks/sessions/templates/prompt-presets),AI/流式域全打/v1→404,typecheck 207 errors(196噪音+11真实bug)。(4) **更新 PROGRESS**: 功能域对比表完成度校正(多个✅→🟡/🔴)、新增前端迁移状态表、新增 GAP-BOARD(16 gaps映射提案+执行顺序)。**用户决策**: refine 回退对齐 v1;WS-B 拆分为独立对齐提案;一个一个做不跳过;完整迁移。**已有提案覆盖**: c25(SSRF config)/c26(citations context)/c27(outputs RAG)/c28(analysis向量)。**新建提案**: c29(refine回退)/c30(sources同步embedding)/c31(QA no-evidence)/c32(studio落盘+端点)/c33(sources端点)/c34(sessions端点)/c35(前端迁移)。 |
