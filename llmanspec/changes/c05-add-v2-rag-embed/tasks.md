...
- [ ] `apps/server/src/rag/chunker.ts` — paragraph-based 分块 (500 chars + 50 overlap)
- [ ] 验证: 1000 字文本分块 → 2 个 chunk

## 2. Embedder

- [ ] `apps/server/src/rag/embedder.ts` — 调用 AI SDK embedding provider
- [ ] 验证: `embedSingle("hello")` 返回 1024-dim Float32Array

## 3. Embed Strategy

- [ ] `apps/server/src/rag/embed-strategy.ts` — 实现 RAGStrategy 接口
...
- [ ] `apps/server/src/rag/cache.ts` — epoch-based 缓存
