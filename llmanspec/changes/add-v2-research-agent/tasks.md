...
## 1. Research Tools
- [ ] `server/src/features/research/tools.ts` — webSearch (SearXNG fetch), analyzeResults, writeReport
- [ ] ai() streamText + tools: {webSearch, analyzeResults, writeReport} + maxSteps=20

## 2. Research Sessions
- [ ] `server/src/features/research/router.ts` — POST create, GET list, POST /:id/approve, POST /:id/stop
- [ ] Research session state: running → awaiting_approval → completed / stopped

## 3. Background Jobs
- [ ] `server/src/features/tasks/queue.ts` — 简单 in-process 任务队列
