...
旧 Deep Research 已 stub（`POST …/research` → 501），无法支撑产品基线（本 change `design.md` Runtime wire + FE 见 `c77-deep-research-ui`）：**ResearchRun SSOT**、原子 tools、可交互图（剪枝/fork）、结构化报告+citation map、显式双开关创建、M1 轻量确认、显式转化（PARAGRAPH+K1 / Source embed）。c75 已完成顶栏 E1 + 直接搜索；本 change **落地后端 Runtime 全量合约与实现**；FE/xyflow 仅占位（`c77-deep-research-ui`）。
...
10. **FE 占位**：不实现 xyflow/Desk；契约与 Zod 可供 `c77-deep-research-ui`；UI 决策见该 change（U1/E1′/F1-CTA/UI-C1/C1 色）。
...
- **本 change `design.md`** — 后端 Runtime wire（原 runtime-spec / PRD §0 后端相关已并入）
- `c77-deep-research-ui` — FE Desk/详情/图 MUST（原 ui-proto 已并入）
