...
- [ ] `apps/server/src/shared/config.ts`：`SlidesPreviewSchema`（`base_url`
      默认 `http://127.0.0.1:3030`、`probe_timeout_ms` 默认 1500）+
      `getSlidesPreview()` typed accessor；挂进 RootConfig
- [ ] `config/app.yaml`：新增 `slides_preview` 节（含 `{{ env.CL_SLIDEV_BASE_URL }}`
      模板与注释）；`packages/shared/src/schemas/env.ts` 声明 `CL_SLIDEV_BASE_URL`
- [ ] 再生 `config/app.schema.gen.json` + `.env.example`（`just gen-env-examples`
      等价生成路径），`check-app-schema` / `check-env-examples` 门禁过

## T2 探测模块 + tools 契约

- [ ] `apps/server/src/shared/slides-availability.ts`：`probeSlidesAvailability()`
...
- [ ] `features/workspace/router.ts`：`diagnostics.slides` 填充探测结果；
      SLIDES tool `enabled` 改为 `available`（engine/activePluginId 填实值）
- [ ] server 测试：假 Slidev 可达 → `slides.available=true` + SLIDES enabled；
      不可达 → `available=false` + `SLIDES_PREVIEW_UNREACHABLE` + hint 非空；
      插件缺失分支（registry 空 impl）→ `SLIDES_PLUGIN_MISSING`

## T3 前端移除 + 诊断呈现

- [ ] `StudioToolsGrid.tsx`：`diagnostics.slides.available === false` 时过滤
      SLIDES 卡片（诊断数据经父级 toolsDiagnostics 传入）
- [ ] DiagnosticsDialog `diagnostics.slides` 渲染复核（已存在，补 unavailable
      分支文案与 hint 展示断言即可）
- [ ] 前端测试：可用 → SLIDES 卡片在；不可用 → 卡片不在 + 其余工具不受影响

## T4 文档与验收

- [ ] README「已知限制」段更新（单二进制形态 SLIDES 自动隐藏，不再是「暂不可用」）
- [ ] `docs/plugins.md` 暴露面表格补一行探测宿主职责备注（防腐烂指针 → design D1）
- [ ] 门禁：typecheck / lint / format / server+shared tests / web Vitest / e2e @p0
