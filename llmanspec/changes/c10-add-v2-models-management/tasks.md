...
- [ ] `apps/server/src/ai/provider-registry.ts` — 按 provider 名 (openai/anthropic/google/deepseek) 创建 provider instance
- [ ] apiKey 从 config/secret.env 读取 ({{secret.OPENAI_API_KEY}})

## 2. Models Endpoint

- [ ] `apps/server/src/features/models/router.ts` — GET /v2/models 返回 [{provider, modelId, displayName}]
