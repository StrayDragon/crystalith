...
mock.module('ai', () => ({
  ...
  generateText: async ({ output }: { output?: unknown }) =>
    output ? { output: MOCK_AI_OBJECT } : { text: 'SSE report' },
  streamText: () => ({
...
describe('research SSE stream', () => {
  it('emits approval_request and done events during a full cycle', async () => {
    ...
    const sseRes = await app.handle(
      new Request(`${BASE}/v2/notebooks/${notebookId}/research/${sessionId}/stream`),
    );
  ...
  it('returns 404 for nonexistent session', async () => {
    const res = await app.handle(
      new Request(`${BASE}/v2/notebooks/${notebookId}/research/99999/stream`),
    );
