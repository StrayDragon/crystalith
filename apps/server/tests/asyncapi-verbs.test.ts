...
describe('c70 AsyncAPI stream verbs', () => {
  it('documents QA POST and research/studio GET with nested paths', () => {
    ...
    expect(doc.channels.researchProgress.address).toBe('/v2/notebooks/{nid}/research/{id}/stream');
    expect(doc.channels.researchProgress.bindings?.http?.method).toBe('GET');
