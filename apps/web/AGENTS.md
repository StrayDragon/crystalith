...
- 剪枝闭包 **B** 在 `research-lab/model/pruneClosure`（与 server `collectResearchPruneClosure` 对齐）；demo `fake/deriveLabState` 内部复用之。变更走 `llmanspec/changes/update-research-prune-cascade`
