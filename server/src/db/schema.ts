...
const json = <T = JsonMetadata>(name: string) =>
  text(name, { mode: "json" }).$type<T>();

/** Non-null JSON column. */
const jsonReq = <T = JsonMetadata>(name: string) => text(name, { mode: "json" }).$type<T>().notNull();
...
export const studioSlides = sqliteTable(
  "studio_slides",
  {
    ...
    stage: text("stage", { enum: ["input", "outline", "markdown"] }).notNull().default("input"),
...
export const researchSessions = sqliteTable(
  "research_sessions",
  {
    ...
    aggregatedResults: text("aggregated_results", { mode: "json" })
      .$type<unknown[] | null>()
      ,
